import { Stats, ScoreResume, ScoreElement } from '../class';
...
export default class Score {
  ...
  Number(): ScoreElement {
    ...
    return new ScoreElement(number, 2, number >= 2, number < 2 ? number * 2 : 6, 6);
  }

  UpperCase(): ScoreElement {
    const uppercase = this._stats.UpperCase();

    return new ScoreElement(uppercase, 2, uppercase >= 2, uppercase < 2 ? uppercase * 2 : 6, 6);
  }

  LowerCase(): ScoreElement {
    const lowercase = this._stats.LowerCase();

    return new ScoreElement(lowercase, 2, lowercase >= 2, lowercase < 2 ? lowercase * 2 : 6, 6);
  }
